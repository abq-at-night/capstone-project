...
class EventTag implements \JsonSerializable {
	...
	public static function getEventTagByEventTagEventId(\PDO $pdo, $eventTagEventId) : \SplFixedArray {
		//Sanitize the eventTagEventId before searching
		try {
	 ...
	 * @throws \TypeError if $pdo is not a PDO connection object
	 */
	public static function getEventTagByEventTagEventIdAndEventTagTagId(\PDO $pdo, $eventTagEventId, $eventTagTagId) : ?EventTag {
		//Sanitize the eventTagEventId and the eventTagTagId before searching.
		try {
			$eventTagEventId = self::validateUuid($eventTagEventId);
		} catch(\InvalidArgumentException | \RangeException | \Exception | \TypeError $exception) {
			throw(new \PDOException($exception->getMessage(), 0, $exception));
		...
		$statement = $pdo->prepare($query);
		//Bind the eventTagEventId and the eventTagTagId to the place holder in the template.
